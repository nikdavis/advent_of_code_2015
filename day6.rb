...
mat = []
1000.times {|i| mat << [0] * 1000}

commands = input.split("\n")
commands.each do |command|
  match = command.match /(?<action>\w+( ?\w+)) (?<i0>\d+),(?<i1>\d+) through (?<i2>\d+),(?<i3>\d+)/
  indices = [ match["i0"],
              match["i1"],
              match["i2"],
              match["i3"]
            ].collect {|i| i.to_i}
  (indices[0]..indices[2]).each do |x|
    (indices[1]..indices[3]).each do |y|
      case match["action"]
      when "turn on"
        mat[x][y] = 1
      when "turn off"
...
lights_on = mat.each.inject(0) do |sum, row|
  sum + row.count(1)
end

puts "#{lights_on} lights on."
...
brightness = mat.each.inject(0) do |sum, row|
  sum + row.inject(:+)
end

puts "Brightness of #{brightness}."
